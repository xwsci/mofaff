...
# restart setting
    if start_cycle > 1:
        ...
        os.chdir(direct)
        module_str_raspa_lmp.raspa_to_lmp('mof_raspa.data','conf.lmp', N_mof_O, type_to_element, remove_H2O)
...
#        file.write(content)
    ...
    with open(f'{direct}/gcmc/simulation.input', 'w') as file:
        for line in lines:
            if remove_H2O == False and line.startswith('RestartFile no'):
                file.write(line.replace('RestartFile no', 'RestartFile yes'))
    ...
    for n in np.arange(start_cycle,N_cycle+1):
        ...
        if remove_H2O == False:
            module_cif_restartfile.convert_H2O_raspa_restartfile('mof_H2O_md.cif','H2O_md.cif','restartfile_ini_H2O', N_mof_O)
